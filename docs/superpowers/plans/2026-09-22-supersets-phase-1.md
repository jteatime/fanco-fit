...
function linkSuperset(list, idA, idB) {
  ...
  /* Merge whole groups, not just the two exercises tapped: when both sides
     are already grouped, moving B alone would strand its former partners in
     a half group — and the Manage link pill sits between adjacent groups, so
     that is reachable. */
  const merging = new Set([a.supersetId, b.supersetId].filter(Boolean));
  for (const e of list) {
    if (e === a || e === b || (e.supersetId && merging.has(e.supersetId))) e.supersetId = id;
  }
...
```javascript
  /* Scoped to the day it came from: ManageView renders all seven DAYS cards
     stacked at once (schedTab only toggles normal/deload, it does not select
     a day), so an unscoped string would show under every day simultaneously. */
  const [linkNote, setLinkNote] = useState(null); /* { day, text } | null */
  const doLink = (day, idA, idB) =>
    update((d) => {
      const res = linkSuperset(mutListFor(d, day), idA, idB);
      setLinkNote(res.raised.length
        ? { day, text: res.raised.map((r) => `${r.name} raised ${r.from} → ${r.to} rounds to match.`).join(" ") }
        : null);
    });
```
...
```jsx
                    ...
                    <Row style={{ justifyContent: "space-between", marginBottom: 6 }}>
                      ...
                      <Row style={{ gap: 4 }}>
                        ...
                        <button className="ll-btn ll-btn-sm" style={{ color: T.neg }}
                          onClick={() => { update((d) => { unlinkSuperset(mutListFor(d, day), g.id); }); setLinkNote(null); }}>unlink</button>
                      </Row>
                    </Row>
                    {/* The editing branch is required: without it ✎ sets state
                        that nothing renders, and a grouped exercise cannot be
                        renamed without unlinking first. Same form as
                        singleExerciseRow, minus the planned-sets stepper — the
                        group's rounds stepper replaces it. */}
                    {g.members.map((m) => (
                      <Row key={m.id} style={{ justifyContent: "space-between", padding: "5px 0" }}>
                        {editing === m.id ? (
                          <div style={{ flex: 1 }}>
                            <Row>
                              <input className="ll-input" style={{ width: 58, flex: "none", textAlign: "center" }} placeholder="🏋️" maxLength={4}
                                value={m.icon || ""} onChange={(e) => update((d) => { findEx(d, day, m.id).icon = e.target.value; })} />
                              ...
                              <button className="ll-btn ll-btn-sm ll-btn-chalk" onClick={() => {
                                ...
                                if (t) update((d) => { findEx(d, day, m.id).name = t; });
                                setEditing(null);
                              }}>Done</button>
                            </Row>
                          </div>
          ...
          {linkNote && linkNote.day === day && (
            <div style={{ color: T.muted, fontSize: 12, marginTop: 6 }}>{linkNote.text}</div>
          )}
```
