...
```javascript
  /* Which unit this sheet's numbers are in. The grid is scoped to the live
     cycle — weekOf is relative to data.cycleStart, so archived-cycle sessions
     fall to negative week indices and appear in no cell, and the download is
     named franco-fit-cycle-<N>.csv — so listing every cycle would describe
     data that is not in the file. Stated unconditionally because a reader of
     a bare number needs it either way.
     Known limit: a unit switch partway through the live cycle cannot be
     expressed here; see migrateUnits' comment on the same gap. */
  rows.push([], ["Units"], ["Cycle", "Unit"]);
  rows.push([data.cycleName || `Cycle ${data.cycleNumber}`, data.unit || "kg"]);
```
